    ...
    async function fetchSales() {
      try {
        ...
        const res = await fetch(
          `https://team6-sales-function-2.azurewebsites.net/api/get_sales?sales_date_from=${year}-${month}-1&sales_date_until=${year}-${month}-${lastDayOfMonth}&location_id=${location_id}`
        );
        if (!res.ok) {
  ...
  return (
    <div className="p-6">
      <div className="text-xl font-semibold mb-4">
        {selectedYear}年{selectedMonth}月の近畿拠点の報告状況
      </div>
      ...
      <div className="flex justify-between items-center flex-wrap gap-2 mb-4">
        ...
        <ExportCsvButton
          data={reports}
          filename={`近畿拠点_${selectedYear}年${selectedMonth}月.csv`}
        />
      </div>
