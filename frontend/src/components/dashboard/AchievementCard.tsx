...
interface AchievementCardProps {
  ...
  target: number | null; // 目標金額（円）: nullなら未設定
}
...
export function AchievementCard({
  ...
  const formattedTarget =
    target !== null ? `¥${target.toLocaleString()}` : "未設定";
  const formattedPercentage =
    target !== null ? `${((percentage ?? 0) * 100).toFixed(1)}%` : "--";
    ...
    <Card>
      ...
      <CardContent>
        <div className="flex justify-between">
          {/* 左側：現在の売上 */}
          <div>
            <div className="text-lg text-muted-foreground">今月の売上</div>
